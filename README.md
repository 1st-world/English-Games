🌍 *Read this in other languages: [English](README_en.md), **한국어***
...
- **로컬에서 실행하기**
  ...
  3. 브라우저 보안 정책(CORS)으로 인해, 단순 파일 열기 대신 로컬 웹 서버를 통해 실행해야 합니다.
      - 예를 들어, VS Code의 'Live Server' 확장 프로그램을 사용할 수 있습니다.
      - 또는 Python이 설치된 경우: `python -m http.server 8000`

## 라이선스 및 크레딧

여기에 제공하는 서드 파티 관련 라이선스 정보는 편의를 위한 요약에 불과하며, 정확하고 법적 효력을 가지는 조건은 각 원본 라이선스 문서 및 공식 출처 등을 직접 확인하기 바랍니다.

- 본 프로젝트의 코드는 기본적으로 [MIT 라이선스](LICENSE)를 따릅니다.
...
- 사용자 웹 브라우저의 '엄격한 추적 방지' 모드 등에서 외부 CDN을 차단하면 시스템 기본 글꼴이 적용되는 현상을 해결하기 위해, 글꼴 파일을 프로젝트 내에 직접 포함하여 로드하고 있습니다. 글꼴의 라이선스는 코드와는 별개이며, 해당하는 정책을 준수해야 합니다.
  - **적용 글꼴: [나눔스퀘어 네오(NanumSquareNeo)](https://campaign.naver.com/nanumsquare_neo)**
  - 문서에 따르면, 이 글꼴은 SIL OFL 1.1 라이선스에 해당합니다.
  - 이 글꼴의 지적 재산권은 네이버와 네이버 문화재단에 있습니다. 개인 및 기업 사용자를 포함한 모든 사용자에게 무료로 제공되며 글꼴 자체를 유료로 판매하는 것을 제외한 상업적인 사용이 가능합니다.

- 일부 아이콘 도입을 위한 서드 파티 에셋을 프로젝트 내에 포함하고 있습니다.
  - **적용 에셋: [Font Awesome Free](https://fontawesome.com)**
  - 문서에 따르면, 아이콘은 CC BY 4.0, 웹 글꼴은 SIL OFL 1.1, 코드는 MIT 라이선스에 해당합니다.
